...
export class NucleusServerResources extends Construct {
	...
	constructor(scope: Construct, id: string, props: NucleusResourcesProps) {
		...
		// Templated secret
		const ovMainLogin = new secretsmanager.Secret(this, "NucleusMainLoginSecret", {
		...
		// Templated secret
		const ovServiceLogin = new secretsmanager.Secret(
			this,
			"NucleusServiceLoginSecret",
			{
				generateSecretString: {
					secretStringTemplate: JSON.stringify({ username: "omniverse" }),
					excludePunctuation: true,
		...
		const instanceRole = new iam.Role(this, "NucleusInstanceRole", {
			assumedBy: new iam.ServicePrincipal("ec2.amazonaws.com"),
			description: "EC2 Instance Role for Nucleus Server",
			managedPolicies: [
		 ...
		 */
		const primaryInstanceName = `${props.stackName}NucleusServerPrimary`;
		this.nucleusServer = new ec2.Instance(this, "NucleusServerPrimaryInstance", {
		 ...
		 */
		const standbyInstanceName = `${props.stackName}NucleusServerStandby`;
		const standbyInstance = new ec2.Instance(this, "NucleusServerStandbyInstance", {
			...
			vpcSubnets: {
				subnets: [
					props.subnets.length > 1 ? props.subnets[1] : props.subnets[0],
				],
			},
			detailedMonitoring: true,
			requireImdsv2: true,
		...
		const nucleusServerConfig = new CustomResource(
			this,
			"NucleusServerConfigCustomResource",
			{
				lambdaName: `${props.stackName}CustomResourceNucleusConfig`,
				lambdaCodePath: path.join(
					__dirname,
					"..",
					"lambda",
					"custom-resources",
					"nucleus-server-config",
				),
