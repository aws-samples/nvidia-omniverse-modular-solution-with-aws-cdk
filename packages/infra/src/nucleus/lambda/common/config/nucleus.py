...
def get_config(artifacts_bucket_name: str, full_domain: str, nucleus_build: str, ov_main_password: str, ov_service_password: str) -> list[str]:
    return f'''
        ...
        cd /opt/ove/base_stack || exit 1
        OVE_DATA_PATH=/var/lib/omni/nucleus-data
        TOKEN=`curl -X PUT "http://169.254.169.254/latest/api/token" -H "X-aws-ec2-metadata-token-ttl-seconds: 60"`
        HOSTNAME=$(curl http://169.254.169.254/latest/meta-data/hostname -H "X-aws-ec2-metadata-token: $TOKEN")
        sudo nst generate-nucleus-stack-env --server-ip $HOSTNAME --reverse-proxy-domain {full_domain} --instance-name nucleus_server --master-password {ov_main_password} --service-password {ov_service_password} --data-root $OVE_DATA_PATH
